...
# (e.g. 'install_eolchecker.sh')
#
# If one argument is given -> 
# If that argument is in the format of #.#.#, downloads the specified
# version of the binary to your current working directory.
# If that argument is something else, downloads the most recently released
# binary to the specified directory.
# (e.g. 'install_eolchecker.sh 0.0.1-rc' or 'install_eolchecker.sh $(go env GOPATH)/bin')
#
# If two arguments are given -> Downloads the specified version of the
# binary to the specified directory.
# (e.g. 'install_eolchecker.sh 0.0.1-rc $(go env GOPATH)/bin')
#
...
release_url=https://api.github.com/repos/Cheunn-Panaa/eol-checker/releases
if [ -n "$1" ]; then
  if [[ "$1" =~ ^[0-9]+(\.[0-9]+){2}$ ]]; then
    version=v$1
    release_url=${release_url}/tags/eol-checker%2F$version
  elif [ -n "$2" ]; then
...
if [ -f "${where}eol-checker" ]; then
  echo "${where}eol-checker exists. Remove it first."
  exit 1
elif [ -d "${where}eol-checker" ]; then
  echo "${where}eol-checker exists and is a directory. Remove it first."
  exit 1
fi
...
RELEASE_URL=$(echo "${releases}" |\
  grep browser_download.*${opsys}-${arch}.tar.gz |\
  cut -d '"' -f 4 |\
  sort -V | tail -n 1 | sed 's/.md5//')
...
${where}eol-checker --version

echo "eol-checker installed to ${where}eol-checker"